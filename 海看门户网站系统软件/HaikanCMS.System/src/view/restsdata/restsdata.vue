      ...
      >
        ...
        <Row :gutter="16">
          <FormItem label="类型" prop="type">
            ...
            >
              <Option
                v-for="item in stores.demo.sources.typeList"
                :value="item.value"
                :key="item.value"
                >{{ item.label }}</Option
              >
        ...
        <Row :gutter="16" v-if="formModel.fields.type=='文本'">
          <FormItem label="文本" prop="accessory">
            <Input
              type="textarea"
        ...
        <fieldset style="margin-bottom: 10px;" v-if="formModel.fields.type=='轮播图'"> 
...
export default {
    ...
    return {
      ...
      stores: {
        demo: {
          ...
          //列表显示
          columns: [
            { type: "selection", width: 50, key: "restsDataUuid" },
            { title: "名称", key: "name", ellipsis: true, tooltip: true },
            { title: "类别", key: "type", ellipsis: true, tooltip: true },
            {
              title: "操作",
              align: "center",
              width: 200,
      ...
      formModel: {
        ...
        rules: {
          name: [
            ...
            { validator: globalvalidateIsNotEmpty, trigger: "blur" },
          ],
          type: [{ type: "string", required: true, message: "请选择类别" }],
        },
      },
    };
  },
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map((x) => x.restsDataUuid);
    }, //删除
  },
  methods: {
    //#region  页面加载
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.restsDataUuid);
    },
    //详情显示
    handleDetail(e) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(e.restsDataUuid);
    },
    //查询当前行信息
    doLoadData(id) {
      GetShow(id).then((res) => {
        if (res.data.code === 200) {
          ...
          if (
            res.data.data.accessory != null &&
            res.data.data.type == "轮播图"
          ) {
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.restsDataUuid);
    },
    doDelete(ids) {
      if (!ids) {
