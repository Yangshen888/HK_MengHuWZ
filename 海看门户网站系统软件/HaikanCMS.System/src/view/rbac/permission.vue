...
export default {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.systemPermissionUuid);
    }
  },
  methods: {
    loadPermissionList() {
    ...
    handleEdit(params) {
      ...
      this.doLoadPermission(params.row.systemPermissionUuid);
    },
    ...
    handleResetFormPermission() {
      ...
      this.formModel.fields.systemMenuUuid = "";
    ...
    doLoadPermission(systemPermissionUuid) {
      loadPermission({ code: systemPermissionUuid }).then(res => {
        this.formModel.fields = res.data.data;
        this.handleRefreshMenuTreeData(this.formModel.fields.systemMenuUuid);
      });
    },
    handleDelete(params) {
      this.doDelete(params.row.systemPermissionUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleMenuTreeSelectChange(nodes) {
      var node = nodes[0];
      if (node) {
        this.formModel.fields.systemMenuUuid = node.guid;
    ...
    handleSearchMenuTreeSelectChange(nodes) {
      var node = nodes[0];
      if (node) {
        this.stores.permission.query.systemMenuUuid = node.guid;
    ...
    handleClearSearchMenuTreeSelection() {
      this.stores.permission.query.systemMenuUuid = "";
