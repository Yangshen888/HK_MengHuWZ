<template>
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="类别名称" prop="newsTypeName">
            <Input v-model="formModel.fields.newsTypeName" placeholder="类别名称" />
          </FormItem>
        </Row>
        <Row :gutter="16">
    ...
    <Drawer title="详情" v-model="formModel1.opened" width="600" :mask-closable="false" :mask="false">
      <Form :model="formModel1.fields" ref="formdispatch22" label-position="top">
        <Row :gutter="16">
          <FormItem label="类别名称" prop="newsTypeName">
            <Input v-model="formModel1.fields.newsTypeName" placeholder="类别名称" :readonly="true" />
          </FormItem>
        </Row>
        <Row :gutter="16">
          <FormItem label="备注" prop="remark">
...
export default {
    ...
    return {
      ...
      stores: {
        userinfo: {
          ...
          //列表显示
          columns: [
            { type: "selection", minwidth: 50, key: "newsTypeUuid" },
            {
              title: "类别名称",
              key: "newsTypeName",
              minWidth: 120,
      ...
      formModel: {
        ...
        rules: {
          newsTypeName: [
            { type: "string", required: true, message: "请输入类别名称" }
          ]
        }
      },
      formModel1: {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.newsTypeUuid);
    } //删除
  },
  methods: {
    //页面加载
    ...
    //右边删除（单个删除）
    handleDelete(row) {
      this.doDelete(row.newsTypeUuid);
    },
    doDelete(ids) {
      if (!ids) {
    ...
    handleEdit(row) {
      ...
      this.formModel.opened = true;
      this.handleResetFormDispatch(); //清空表单
      this.doLoadData(row.newsTypeUuid);
    },
    //右边详情
    handleDetail(row) {
